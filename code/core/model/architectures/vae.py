...
class LVAE(VLAE):
	...
	def _setup_encoder_call(self):
		...
		def call(inputs,use_mean=False,**hparams):
			encoder_ls_inputs = []
			pred = inputs
			latent_space=[]
			# Bottom up pass 
			for i,layer in enumerate(enc_layers[:-1]):
				...
				if (i in enc_latent_connections_map) or (i-len(enc_layers) in enc_latent_connections_map): # intermediate latent layers 
					...
					#encoder_ls_inputs.append(pred*self.alpha[ladder_num])
					encoder_ls_inputs.append(pred)
			#top_layer = list(self.encoder.convert_layer_to_latent_space(layers[-1](pred*self.alpha[-1])))
	...
	def _setup_decoder_call(self):
		dec_layers = self.decoder.layers
		dec_latent_connections = {lc[1]:i for i,lc in enumerate(self.latent_connections)}
		def call(latent_space_samples=None, latent_space=None, return_ls=False,use_mean=False,**hparams):
			""" decode.
	...
	def call(self, inputs, **kw):
		...
		reconstruction = self.decoder(None, latent_space=self.latent_space,**kw)
		#TBD/TODO: prior for top layer is hard coded here as zeros
	...
	def regularizer(self, latent_space, prior_space, **hparams):
		...
		for i,ls,ps in zip(range(len(latent_space)), latent_space, prior_space):
			# select the prior to be a specific distribution
			reg_loss,kld=self.layer_regularizer(*ls,*ps[1:], return_kld=True, layer_num=i, 
...
class LatentMaskLVAE:
	def __init__(self, num_latents,is_mask_sample=False):
...
class ProVLAE(ProVLAEBase):
	...
	def call(self, inputs, alpha=None, beta=None, latent_mask=None, **kw):
		...
		# get reconstruction and regularization loss, also applies shortcut routing to latent_space
		losses = self.provlae_regularizer(self.latent_space, self.alpha, beta, **kw)
		for loss in losses:
			self.add_loss(loss)
	...
	def create_regularized_routing(self,latent_space,routing):
		...
		for i,ls in zip(list(range(len(latent_space)))[::-1], latent_space[::-1]):
			...
			for k,v in routing.items():
				...
				# shortcut routing, resample.
				# TBD: latent shortcut doesn't account for multiple parents
				self.latent_space[i][1] += mean 
				self.latent_space[i][2] += logvar
				# TBD: latent resample is hard coded
				self.latent_space[i][0]=tf.where(
					mask,
					tf.exp(0.5*self.latent_space[i][2])*tf.random.normal(tf.shape(self.latent_space[i][2]))+self.latent_space[i][1],
					self.latent_space[i][0],
					)
